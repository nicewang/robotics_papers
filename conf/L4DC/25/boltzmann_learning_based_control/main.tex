...
\author{
  ...
  Topic: Boltzmann Learning, Non-Stationary Environments, Learning-based Control \\
}
...
\begin{itemize}
  ...
  \begin{itemize}
    \item Conceptually, ${\text{Regret}_K(\rho) = \underbrace{ \frac{1}{K} \sum_{k=1}^{K} \mathbb{E}[J(a_k, \omega_k)] }_{\text{Actual Cost}} - \underbrace{ \frac{1}{K} \sum_{k=1}^{K} \mathbb{E}[J(\rho(y_k), \omega_k)] }_{\text{Benchmark Cost}} \leq \alpha_0}$
  \end{itemize}
  \item It is \textbf{computationally much more efficient} and requires significantly \textbf{less memory} than competing \textit{machine learning approaches} (SVM and Neural Network, which must be continuously retrained on sliding windows of past data to handle non-stationarity).
\end{itemize}
