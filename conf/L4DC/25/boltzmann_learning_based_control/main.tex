...
There is a trick: since we can not obtain the actual cost (i.e. $J_{k}$) when in decision making, we use an approximation: \textbf{``Regard Energy as cost''} \textbf{\textit{(Using energy instead of cost.)}}. Thus, we regard energy $E$ as $\text{Virtual} E$ there.
